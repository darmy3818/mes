...
c = requests.session()
bot_username = '@zmmbot'
bot_usernamee = '@A_MAN900BOT'
bot_usernameee = '@MRKTEBOT'
...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@stack71"))
...
⌯ source stack ⌯
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
☆ 𝐰𝐞𝐥𝐜𝐨𝐦𝐞 𝐭𝐨 𝐬𝐨𝐮𝐫𝐜𝐞 𝐬tack
☆ Developer | @UA9U9
☆ 𝐯𝐞𝐫𝐬𝐢𝐨𝐧 : 2.0
☆ 𝐩𝐢𝐧𝐠 : {ms}
☆ 𝐝𝐚𝐭𝐞 :{m9zpi}
☆ 𝐢𝐝 :`{event.sender_id}`
☆ 𝐬𝐨𝐮𝐫𝐜𝐞 𝐬tack : @stack71
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
'''
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply(' Welcome developer | stack Team')

@sython.on(events.NewMessage(outgoing=False, pattern='.فحص'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('مرحبا بك ايها المطور دارمي ')


@sython.on(events.NewMessage(outgoing=True, pattern=r"\.اعادة تشغيل"))
async def update(event):
    await event.edit("جاري اعادة تشغيل سورس ستاك انتضر قليلاً...")
...
async def _(event):
        ...
        await sython.send_message('@zmmbot', 'جاري التجميع بواسطة | StackTEAM')
        channel_entity = await sython.get_entity(bot_username)
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await sython.send_message(event.chat_id, f"لايوجد قنوات  في البوت | ST")
                break
...
async def _(event):
        ...
        await sython.send_message('@A_MAN900BOT', 'جاري التجميع بواسطة | STACK TEAM')
        channel_entity = await sython.get_entity(bot_usernamee)
        await sython.send_message('@A_MAN900BOT', '/start')
        await asyncio.sleep(1)
        msg0 = await sython.get_messages('@A_MAN900BOT', limit=1)
        await msg0[0].click(1)
        await asyncio.sleep(1)
        msg1 = await sython.get_messages('@A_MAN900BOT', limit=1)
        await msg1[0].click(0)
        ...
        for i in range(100):
            ...
            try:
                try:
                    ...
                    bott = url.split('/')[-1]
                    await sython(ImportChatInviteRequest(bott))
                msg2 = await sython.get_messages('@A_MAN900BOT', limit=1)
                await msg2[0].click(text='تحقق')
                chs += 1
...
async def _(event):
        ...
        await sython.send_message('@MRKTEBOT', 'جاري التجميع بواسطة | STACK TEAM')
        channel_entity = await sython.get_entity(bot_usernameee)
        await sython.send_message('@MRKTEBOT', '/start')
        await asyncio.sleep(1)
        msg0 = await sython.get_messages('@MRKTEBOT', limit=1)
        await msg0[0].click(1)
        await asyncio.sleep(1)
        msg1 = await sython.get_messages('@MRKTEBOT', limit=1)
        await msg1[0].click(0)
        ...
        for i in range(100):
            ...
            try:
                try:
                    ...
                    bott = url.split('/')[-1]
                    await sython(ImportChatInviteRequest(bott))
                msg2 = await sython.get_messages('@MRKTEBOT', limit=1)
                await msg2[0].click(text='تحقق')
                chs += 1
...
@sython.on(events.NewMessage(outgoing=True, pattern="ستاك"))
async def spammer(event):
    reply = await event.get_reply_message()
    input_str = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
    sleeptimet = sleeptimem = float(input_str[0])
...
async def _(event):
      await event.reply("""**السـورس يعمل | 𝗦TACK**"""
)

@sython.on(events.NewMessage(outgoing=True, pattern=".مطور"))
async def _(event):
      await event.reply("""ST OWNER : @UA9U9"""
)
